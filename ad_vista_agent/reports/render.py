...
def render_html_report(
    ...
    *,
    ...
    for dimension in DIMENSION_LABELS:
        ...
        featured = " featured" if dimension == MarketingDimension.SELLING_POINT else ""
        sections.append(
            f'<section class="dimension{featured}" id="{anchor}"><div class="section-heading">'
...
<style>
:root{{--ink:#17221f;--deep:#173b34;--paper:#f3f1ea;--paper-light:#fffdfa;--line:#d9d9d1;--muted:#68736d;--orange:#d2643c;--amber:#9b722c;--red:#b0524a;--green:#2f7259;--shadow:0 12px 30px rgba(26,48,39,.07)}}
...
.layout{{display:grid;grid-template-columns:232px minmax(0,1fr);min-height:100vh}}.rail{{position:sticky;top:0;height:100vh;padding:30px 22px;background:var(--deep);color:#f8f3e8;display:flex;flex-direction:column}}.brand{{font:750 14px/1.2 ui-monospace,monospace;letter-spacing:.08em}}.brand small{{display:block;margin-top:8px;color:#b8c7bd;font:12px/1.4 inherit;letter-spacing:0}}.rail-title{{margin:52px 0 12px;color:#aebfb4;font-size:10px;font-weight:750;letter-spacing:.16em}}nav{{display:grid;gap:3px}}nav a{{display:flex;align-items:center;gap:9px;padding:9px 5px;color:#e3e9df;text-decoration:none;font-size:13px;border-left:2px solid transparent}}nav a:hover{{color:#fff;border-left-color:var(--orange);background:rgba(255,255,255,.06)}}nav em{{margin-left:auto;color:#9eb2a5;font-style:normal;font-size:11px}}.nav-dot{{width:6px;height:6px;border-radius:50%;background:#879f91}}.rail-foot{{margin-top:auto;color:#9eb2a5;font-size:11px;line-height:1.6}}
main{{min-width:0;max-width:1180px;width:100%;padding:52px 6vw 90px}}header{{position:relative;padding:30px 0 40px;border-bottom:1px solid var(--line)}}.kicker{{margin:0 0 17px;color:var(--orange);font:750 10px/1 ui-monospace,monospace;letter-spacing:.17em}}h1{{max-width:820px;margin:0 0 16px;font-size:46px;line-height:1.15;font-weight:760;letter-spacing:0}}.summary{{max-width:760px;margin:0;color:#526159;font-size:16px}}.header-row{{display:flex;align-items:end;justify-content:space-between;gap:24px;margin-top:30px}}.status{{display:inline-flex;align-items:center;gap:7px;padding:6px 11px;border-radius:4px;font-size:12px;font-weight:750;white-space:nowrap}}.status::before{{content:"";width:6px;height:6px;border-radius:50%;background:currentColor}}.status.pass{{color:var(--green);background:#e6f0e9}}.status.review{{color:var(--amber);background:#f3ecda}}.status.fail{{color:var(--red);background:#f4e6e3}}
.scoreboard{{display:grid;grid-template-columns:repeat(4,minmax(86px,1fr));gap:1px;max-width:610px;border:1px solid var(--line);background:var(--line);box-shadow:var(--shadow)}}.score{{padding:13px 15px;background:var(--paper-light)}}.score strong{{display:block;font-size:22px;line-height:1.15}}.score span{{display:block;margin-top:4px;color:var(--muted);font-size:11px}}
.dimension{{padding-top:56px;scroll-margin-top:18px}}.section-heading{{display:flex;align-items:baseline;gap:12px;margin-bottom:17px}}h2{{margin:0;font-size:21px;line-height:1.2}}.section-index{{color:var(--orange);font:750 12px ui-monospace,monospace}}.section-count{{margin-left:auto;color:var(--muted);font-size:12px}}
.dimension.featured .section-heading{{padding-bottom:12px;border-bottom:2px solid var(--ink)}}.dimension.featured h2::after{{content:"重点";display:inline-block;margin-left:9px;padding:3px 6px;background:var(--orange);color:#fff;font:750 9px/1 ui-monospace,monospace;vertical-align:middle;letter-spacing:.08em}}
.insight{{position:relative;margin:0 0 16px;padding:24px 26px 27px;background:var(--paper-light);border:1px solid var(--line);border-top:3px solid var(--green);box-shadow:var(--shadow)}}.insight.review{{border-top-color:var(--amber)}}.insight.fail{{border-top-color:var(--red)}}.insight-head{{display:flex;align-items:center;justify-content:space-between;gap:14px}}.insight-id{{color:var(--muted);font:600 11px ui-monospace,monospace;letter-spacing:.04em}}h3{{max-width:850px;margin:14px 0 8px;font-size:20px;line-height:1.45}}.reasoning{{max-width:850px;margin:0;color:#53635b}}.meta{{display:flex;flex-wrap:wrap;gap:10px 18px;margin-top:16px;color:var(--muted);font-size:12px}}.meta span+span{{position:relative}}.meta span+span::before{{content:"/";position:absolute;left:-11px;color:#b4b5aa}}
.subhead{{margin-top:22px;color:var(--muted);font-size:10px;font-weight:750;letter-spacing:.12em}}.evidence-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px;margin-top:10px}}.evidence-card{{overflow:hidden;border:1px solid var(--line);background:#f7f4ed}}.evidence-card img{{display:block;width:100%;aspect-ratio:16/10;object-fit:cover;background:#e5e1d7}}.evidence-copy{{padding:11px 12px}}.evidence-label{{display:flex;justify-content:space-between;gap:8px}}code{{font-family:ui-monospace,monospace;font-size:.8em}}.time{{color:var(--orange);font:650 .76em ui-monospace,monospace}}.evidence-card p{{margin:7px 0 8px;line-height:1.5}}small{{color:var(--muted);font-size:11px}}.empty-state{{margin-top:9px;padding:12px;background:#f1ede4;color:var(--muted);font-size:12px}}.findings{{margin-top:18px;padding-top:14px;border-top:1px dashed var(--line)}}.findings .subhead{{margin:0}}.findings ul{{margin:8px 0 0;padding-left:20px;color:var(--muted);font-size:12px}}.findings .warning{{color:var(--amber)}}.findings .error{{color:var(--red)}}
.unknowns{{margin-top:58px;padding:24px 26px;background:#ebe6d9;border-top:3px solid var(--orange);scroll-margin-top:18px}}.unknowns ul{{display:grid;gap:8px;margin:0;padding:0;list-style:none;color:#53635b;font-size:13px}}.unknowns li{{display:flex;gap:10px}}.unknown-mark{{display:inline-grid;place-items:center;flex:none;width:18px;height:18px;border-radius:50%;background:#e6c6a7;color:#7d482d;font:750 11px/1 ui-monospace,monospace}}.unknowns.empty p{{margin:0;color:var(--muted);font-size:13px}}footer{{margin-top:56px;padding-top:18px;border-top:1px solid var(--line);color:var(--muted);font-size:11px}}
@media(max-width:780px){{.layout{{display:block}}.rail{{position:relative;height:auto;padding:20px 18px 16px}}.rail-title{{margin:22px 0 8px}}nav{{display:flex;overflow:auto;gap:8px;padding-bottom:3px}}nav a{{flex:none;padding:6px 8px;border:1px solid #3e6358;font-size:12px}}nav a:hover{{border-color:var(--orange)}}.rail-foot{{display:none}}main{{padding:30px 18px 58px}}h1{{font-size:34px}}.header-row{{display:block;margin-top:25px}}.scoreboard{{margin-top:18px;grid-template-columns:repeat(2,1fr);max-width:none}}.score:first-child{{grid-column:span 2}}.dimension{{padding-top:40px}}.insight{{padding:19px 17px 21px}}h3{{font-size:18px}}}}
@media print{{.layout{{display:block}}.rail{{display:none}}main{{max-width:none;padding:0}}.dimension{{break-before:page}}.insight{{break-inside:avoid;box-shadow:none}}.evidence-card{{break-inside:avoid}}}}
